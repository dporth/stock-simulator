...
class AccountDAO():
    ...
    def get_accounts_by_user_id(self, user_id):
        ...
        with self._db.session_scope() as session:
            return session.query(Account, User, Stock, AccountValue).join(AccountValue, isouter=True).join(Stock).join(User).filter(User.user_id==user_id).order_by(Account.account_id.desc(), AccountValue.valid_from.asc())
